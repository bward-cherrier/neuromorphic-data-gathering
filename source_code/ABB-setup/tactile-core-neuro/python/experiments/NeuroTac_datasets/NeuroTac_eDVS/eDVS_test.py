...
from core.sensor.tactile_sensor_neuro import NeuroTac_eDVS

def make_meta(meta_file, 
              sensor = 'eDVS', # Sensor type: DAVIS240, eDVS or DVXplorer           
              ...
              n_trials = 4, # Number of taps at each pose
              ):
...
def make_sensor():  
  return NeuroTac_eDVS(port=7777)
...
def main():
  ...
  data_dir = os.path.join(os.environ['DATAPATH'], 'NeuroTac_eDVS', os.path.basename(__file__)[:-3], os.path.basename(__file__)[:-3]+'_'+time.strftime('%m%d%H%M'))
