...
#    del meta['video_dir'], meta['video_df_file'],
...
def main():
    ...
    train_meta_file = os.path.join(home_dir, 'collect5d_rand_03091020', 'meta.json')
    valid_meta_file = os.path.join(home_dir, 'collect5d_rand_03091459', 'meta.json')
    
    # hyperopt search parameters
    space = {
        'num_conv_layers': scope.int(hp.quniform(label='num_conv_layers', low=1, high=5, q=1)),
        'num_conv_filters': scope.int(2 ** (hp.quniform(label='num_conv_filters', low=1, high=8, q=1))),
        'num_dense_layers': scope.int(hp.quniform(label='num_dense_layers', low=1, high=5, q=1)),
        'num_dense_units': scope.int(2 ** (hp.quniform(label='num_dense_units', low=1, high=8, q=1))),
