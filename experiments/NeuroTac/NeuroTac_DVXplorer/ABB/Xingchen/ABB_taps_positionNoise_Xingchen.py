...
def make_meta(robot_tcp = [0, 0, 59, 0, 0, 0], # Size of the TacTip (tcp = tool center point)
              ...
              tap_move = [[0, 0, 2.5, 0, 0, 0], [0, 0, 0, 0, 0, 0]],   # Sequence of positions for the tap relative to the robot's current pose (positive z goes down)
              # obj_poses = [[0, y, 0, 0, 0, 0]for y in range(0, 16, 15)], # Position of the different objects/poses relative to the work frame origin
              # obj_poses = [[-19*x, 22.5*y, 0, 0, 0, 0] for x in range(3) for y in range(9)],
              obj_poses = [[-19*x + r_bias*np.cos(np.pi*theta/4), 22.5*y + r_bias*np.sin(np.pi*theta/4), 0, 0, 0, 0] for x in range(3) for y in range(9) for theta in range(8)],
