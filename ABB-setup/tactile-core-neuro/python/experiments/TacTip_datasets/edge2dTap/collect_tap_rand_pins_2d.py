...
def make_sensor(exposure=-6):
    camera=CvVideoCamera(source=0, exposure=exposure)
    for _ in range(5): 
...
def collect_tap(image_dir, image_df_file, num_frames, tap_move, robot_tcp, 
    ...
    with make_robot(ip) as robot, make_sensor(exposure) as sensor:       
...
#        sensor.process(num_frames=1+num_frames, outfile=os.path.join(image_dir, 'image_init.png'))      
        ...
        for index, row in image_df.iterrows():
            ...
            pins = sensor.process(num_frames=1+num_frames, 
                           outfile=os.path.join(image_dir, sensor_image))
            robot.move_linear(tap_move[1])
            robot.coord_frame = work_frame
            
            # save pins data (same directory as images)
            np.save(os.path.join(image_dir, sensor_image).replace('.png','').replace('image_','pins_'), 
                    pins[:, :, 1])
