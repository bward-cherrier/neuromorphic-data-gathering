...
def make_meta(home_dir, meta_file,
              ...
              crop = [160, 70, 490, 400],
              threshold = (11, -5), 
              poses_rng = [[-5, 0, 1, -15, -15, -45], [5, 0, 5, 15, 15, 45]], 
...
def collect(image_df, home_dir, image_dir, num_frames, robot_tcp, 
            ip, size, crop, threshold, exposure,
            base_frame, home_pose, work_frame, linear_speed, angular_speed, **kwargs):    
    os.makedirs(os.path.join(home_dir, image_dir))
#    image_df = pd.read_csv(os.path.join(home_dir, image_df_file))
    ...
    with make_robot(ip) as robot, make_sensor(size, crop, threshold, exposure) as sensor:       
        ...
        sensor.process(num_frames=1+num_frames, 
                outfile=os.path.join(home_dir, image_dir, 'image_init.png'))      
        ...
        for index, row in image_df.iterrows():
            ...
            sensor.process(num_frames=1+num_frames, 
                           outfile=os.path.join(home_dir, image_dir, sensor_image))
...
meta_file = os.path.join('edge5d', os.path.basename(__file__)[:-3]+'_'+time.strftime('%m%d%H%M'), 
                         'meta.json')
...
# Temporary home dir to not collect in onedrive
meta['home_dir'] = os.path.join(os.environ['TEMPPATH'], 'images'+time.strftime('%m%d%H%M'))

# Collect data
collect(image_df, **meta)

# Zip data
shutil.make_archive(os.path.join(home_dir, meta['image_dir']), 'zip', os.path.join(meta['home_dir'], meta['image_dir']))
shutil.rmtree(os.path.join(meta['home_dir'], meta['image_dir']))
