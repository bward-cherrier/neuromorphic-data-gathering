...
def make_video_df(poses_rng, moves_rng, num_poses, obj_poses, video_df_file, **kwargs):
    # generate random poses
    np.random.seed()
    poses = np.random.uniform(low=poses_rng[0], high=poses_rng[1], size=(num_poses, 6))
    poses = poses[np.lexsort((poses[:,3], poses[:,5]))]
    ...
    for i in range(num_poses * len(obj_poses)):
        ...
        move = moves[i_pose, :]
        video_df.loc[i] = np.hstack((video_file, i_obj+1, i_pose+1, pose, move))
    video_df.to_csv(video_df_file, index=False)
    
def collect(video_dir, video_df_file, num_frames, robot_tcp, base_frame, ip,
            home_pose, work_frame, linear_speed, angular_speed, **kwargs):    
