...
def make_meta (robot_tcp = [0, 0, 120, 0, 0, 0], # Size of the TacTip (tcp = tool center point)
              ...
              obj_poses = [[0, 0, 0, 0, 0, ang] for ang in range(0,179,10)], # Position of the different objects/poses relative to the work frame origin
              n_trials = 10, # Number of taps at each pose
...
def collect_data(collect_dir, video_dir, events_dir, tap_move, obj_poses, depths, home_pose, base_frame, work_frame,robot_tcp,linear_speed,angular_speed,n_trials,**kwargs):
    ...
    for trial_idx in range (n_trials):
      ...
      for pose_idx in range(len(obj_poses)):
        ...
        for depth in depths:
          ...
          tap_move = [[0, 0, depth, 0, 0, 0], [0, 0, 0, 0, 0, 0]]
          sensor.reset_variables()
          events_on_file = os.path.join(events_dir,'taps_trial_' +str(trial_idx) +'_pose_'  + str(pose_idx) + '_depth_' + str(depth) + '_events_on')
          events_off_file = os.path.join(events_dir,'taps_trial_' +str(trial_idx) +'_pose_'  + str(pose_idx) + '_depth_' + str(depth) + '_events_off')
          events_video = os.path.join(video_dir,'event_stream_trial_' +str(trial_idx) +'_pose_'  + str(pose_idx) + '_depth_' + str(depth) + '.mp4')
          acc_video = os.path.join(video_dir,'accumulator_trial_' +str(trial_idx) +'_pose_'  + str(pose_idx) + '_depth_' + str(depth) + '.mp4')                    
          ...
          # Tap
          print("Trial " + str(trial_idx+1) + "/" + str(n_trials) + " Angle " + str(5*(pose_idx+1)) + "/" + str(180)+ ' Depth ' + str(depth))  
          robot.coord_frame = base_frame
